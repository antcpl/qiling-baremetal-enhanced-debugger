...
## Fuzzing example : [fuzzing_example](./fuzzing_example/)

This example is here to demonstrate how fuzzing may be performed using UnicornAFL and Qiling together. The targeted binary is a firmware that receives commands and parse them via USART peripheral. It is a really simple firmware presenting no vulnerability and used here for the sake of the demonstration. It has been compiled for ARM Cortex M3 target. 

**Description of the files** : 

- **fuzz.py** : Qiling python script, performs the emulation initialisation and also holds the UnicornAFL API calls. The whole script is commented. Two main things are interesting : the possibility to control the emulation using Qiling and mix it with the UnicornAFL API and also how is it possible to generate one fuzzing input for the binary and consume at different times. 
- **fuzz.sh** : the bash script that executes the afl-fuzz command with all the needed parameters. You will find the description of each parameter in the AFL++ white paper available on Internet. 
- **execution.bin** : the snapshot used to start the fuzzing from. This is the resulting binary of the use of our enhanced baremetal debugger. 
- **afl_inputs/sample** : a file gathering an input example that is not crashing the binary, AFL uses it to test the "classical" behaviour of the binary.  
- **afl_outputs** : the directory that will holds all the different outputs generated by the fuzzing process 
- **protocol.dict** : a dictionary that is holding different valid command examples that AFL will use to start mutation from. 

**Execution of the example** : 
1. You should have installed AFL++ and UnicornAFL as described at the end of this : [README.md](../../../README.md)
2. You must modify the path to the afl-fuzz binary in ```fuzz.sh``` to math to your own setup. 
